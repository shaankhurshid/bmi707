...
x = Conv1D(filters=1,kernel_size=20,strides=5)(input)
x = MaxPool1D(2)(x)
x = Dropout(0.1)(x)
x = Conv1D(filters=100,kernel_size=10,strides=5)(x)
x = MaxPool1D(2)(x)
...
# Early stopping
es = EarlyStopping(monitor='val_loss', min_delta=-0.001, patience=20, verbose=1, mode='auto', baseline=None, restore_best_weights=True)

model.fit(x_train_final, y_train_final, epochs =50, verbose = 1,shuffle=True,
